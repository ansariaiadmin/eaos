...
echo -e "${BOLD}${BLUE}📊 وضعیت eaos — Status Check v3.1.0 — تاریکی روشن شد${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
    ...
    if [ -n "$TELEGRAM_BOT_TOKEN" ] && [ -n "$TELEGRAM_CHAT_ID" ]; then
      ...
      if command -v curl &> /dev/null; then
        ...
        if curl -sf https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/getMe -o /dev/null 2>&1; then
          ...
          # Check chat
          if curl -sf -X POST -H "Content-Type: application/json" -d "{\"chat_id\":\"$TELEGRAM_CHAT_ID\",\"text\":\"🔔 تست eaos — Status Check — $(date)\"}" https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage -o /dev/null 2>&1; then
