...
curl -X POST http://localhost:3000/api/auth/login \
  ...
  -d '{"email":"admin@eaos.dev","password":"Admin@123"}'
```
...
{
  "token": "jwt-token-...",
  "user": { "email": "admin@eaos.dev", "role": "admin" }
}
```
...
```json
{
  ...
  "providers": {
    ...
    "telegram": { "status": "ok", "bot": "@eaos_notif_bot" }
  },
  "notifications": {
...
1. تلگرام → @BotFather → /newbot → اسم → یوزرنیم (مثل eaos_notif_bot)
