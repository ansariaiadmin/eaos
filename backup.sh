...
BACKUP_FILE="$BACKUP_DIR/eaos-backup-$TIMESTAMP.tar.gz"
BACKUP_ENCRYPTED="$BACKUP_DIR/eaos-backup-$TIMESTAMP.tar.gz.enc"
...
if command -v docker &> /dev/null && docker compose ps 2>/dev/null | grep -q "Up"; then
  ...
  docker compose exec -T db pg_dump -U eaos eaos 2>/dev/null > /tmp/eaos-db-$TIMESTAMP.sql || echo "DB dump fail — شاید DB خاموشه"
  ok "DB dump — /tmp/eaos-db-$TIMESTAMP.sql"
else
...
if [ -f /tmp/eaos-db-$TIMESTAMP.sql ]; then
  tar -rf $BACKUP_FILE /tmp/eaos-db-$TIMESTAMP.sql 2>/dev/null || true
...
ls -t $BACKUP_DIR/eaos-backup-*.tar.gz* 2>/dev/null | tail -n +8 | xargs rm -f 2>/dev/null || true
