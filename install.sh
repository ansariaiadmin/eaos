...
echo -e "${CYAN}"
cat <<'BANNER'
EAOS — v3.1.0 — تاریکی روشن شد — پشتیبانی صفر
سیستم عامل ایجنت + LLM + ترید + صدا + ناتیف — تاریکی روشن شد
LLM + Trading + SMS + Telegram
BANNER
echo -e "${NC}"
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  🧙‍♂️ جادوگر نصب EAOS v3.1.0 — تاریکی روشن شد${NC}"
echo -e "${BLUE}  LLM + Trading + SMS + Telegram${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${YELLOW}سلام! 👋 سیستم عامل ایجنت + LLM + ترید + صدا + ناتیف — تاریکی روشن شد${NC}"
...
if [ "eaos" = "universal-document-os" ]; then
...
if [ "$TELEGRAM_ENABLED" = "yes" ]; then
  ...
  if [ -n "$TELEGRAM_BOT_TOKEN" ] && command -v curl &> /dev/null; then
    ...
    if [ -n "$TELEGRAM_CHAT_ID" ]; then
      curl -sf -X POST -H "Content-Type: application/json" -d "{\"chat_id\":\"$TELEGRAM_CHAT_ID\",\"text\":\"🧪 تست EAOS v3.1.0 — تاریکی روشن شد — $(date)\"}" https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage -o /dev/null 2>&1 && ok "Telegram پیام تست فرستاده شد — چک کن — تاریکی روشن شد" || err "Telegram پیام fail — Chat ID چک کن — ربات استارت کردی؟ — تاریکی روشن شد"
    fi
  fi
...
if [ "$SKIP_ENV" = "true" ]; then
  ...
  cat > .env <<EOF
# EAOS — .env — v3.1.0 — تاریکی روشن شد — $(date)
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  🎉 جادو تمام! EAOS آماده — تاریکی روشن شد! 🎉${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
echo -e "${BOLD}${BLUE}📍 دسترسی:${NC}"
echo -e "${GREEN}  🌐 http://localhost:3000 — سیستم عامل ایجنت + LLM + ترید + صدا + ناتیف — تاریکی روشن شد${NC}"
