...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  EAOS — Enterprise Agent OS${NC}"
echo -e "${BLUE}  نصب خودکار - Auto Installer v0.9.2${NC}"
echo -e "${BLUE}  سیستم عامل ایجنت سازمانی${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
if [[ "FastAPI + React Flow + Temporal" == *"Python"* ]]; then
...
if [[ "FastAPI + React Flow + Temporal" == *"Node"* ]] || [[ "FastAPI + React Flow + Temporal" == *"Next"* ]] || [[ "FastAPI + React Flow + Temporal" == *"Nest"* ]]; then
...
echo -e "  آدرس / URL: http://localhost:8000 (API) و http://localhost:3000 (Web)"
echo -e "  ورود / Login: بدون لاگین پیش‌فرض، توکن در .env"
